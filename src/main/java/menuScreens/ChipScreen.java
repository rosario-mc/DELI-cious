...
public class ChipScreen implements DisplayScreens {
    ...
    public void display() {
        ...
        while (run) {
            ...
            switch (choice) {
                case "1", "HOT CHEETOS" -> {
                    order.setChips(List.of(new Chip("Hot Cheetos")));
                    run = false;
                }
                case "2", "LAYS" -> {
                    order.setChips(List.of(new Chip("Lays")));
                    run = false;
                }
                case "3", "FRITOS" -> {
                    order.setChips(List.of(new Chip("Fritos")));
                    run = false;
                }
                case "4", "DORITOS" -> {
                    order.setChips(List.of(new Chip("Doritos")));
                    run = false;
                }
